from app import (
    app,
    HEADER_PATTERN, HISTORY_PATH, LABEL_HEADER, OUTPUT_FOLDER, PAGES, UPLOAD_FOLDER
)
...
def index():
    response = render_template(
        template_name_or_list=PAGES['index']['template'],
        page_css=PAGES['index']['css'],
        page_title=PAGES['index']['title'],
        page_scripts=PAGES['index']['scripts'],
...
def process_spreadsheet():
    ...
    if request.method == 'POST':
        ...
        if spreadsheet.filename.endswith('.xlsx'):
            ...
            # Caso a planilha seja válida...
            if len(header) == len(HEADER_PATTERN) and len(labels):
                ...
                # Indicar a não ocorrência de uma mensagem flash
                flash_msg = False
            # Do contrário...
            else:
                # Remover a planilha do diretório
                remove(file_path)
...
def show_labels(id: int):
    try:
        ...
        response = render_template(
            template_name_or_list=PAGES['labels']['template'],
            page_css=PAGES['labels']['css'],
            page_title=PAGES['labels']['title'],
            page_scripts=PAGES['labels']['scripts'],
...
def download_file(type: str, id: int):
    directory = join(getcwd(), 'output', type)
    print(directory)
